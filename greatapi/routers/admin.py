...
async def fetch_dashboard_page(request: Request) -> HTMLResponse:
    items = [
...
async def fetch_login_page(request: Request) -> HTMLResponse:
    return templates.TemplateResponse('authentication/login.html', {'request': request})


@router.get('/account', response_class=HTMLResponse)
async def fetch_account_page(request: Request) -> HTMLResponse:
    return templates.TemplateResponse('dashboard/account.html', {'request': request})


@router.get('/settings', response_class=HTMLResponse)
async def fetch_settings_page(request: Request) -> HTMLResponse:
...
async def fetch_model_items_page(request: Request, group_name: str, group_item: str) -> HTMLResponse:
    titles = []
    items = []
    sidebar_groups = []
...
async def fetch_add_item_page(request: Request) -> HTMLResponse:
...
async def fetch_app_page(request: Request, group_name: str) -> HTMLResponse:
    items = []
...
async def fetch_history_page(request: Request) -> HTMLResponse:
    return templates.TemplateResponse(
...
async def fetch_delete_user_page(request: Request) -> HTMLResponse:
...
async def fetch_visualization_page(request: Request) -> HTMLResponse:
