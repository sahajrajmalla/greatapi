...
def jinja_tpl_to_py(input_file_name: str, base_copy_path: Path, base_paste_path: Path, name: str, type: str) -> None:
    ...
    with open(base_copy_path.joinpath(input_file_name), "r") as input_template:
...
def setup_app_template(base_copy_path: Path, base_paste_path: Path, app_name: str) -> None:
    ...
    for child in base_copy_path.iterdir():
        ...
        jinja_tpl_to_py(child_path.name, base_copy_path, base_paste_path, app_name, setup_type)
    ...
    for child in base_copy_path.iterdir():
        ...
        if child_path.is_dir():
            for inner_child in child_path.iterdir():
                ...
                jinja_tpl_to_py(inner_child_path.name, child_path, paste_project_path, project_name, setup_type)
        else:
            jinja_tpl_to_py(child_path.name, base_copy_path, base_paste_path, project_name, setup_type)
